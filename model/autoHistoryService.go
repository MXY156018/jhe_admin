...
func (autoCodeHistoryService *AutoCodeHistoryService) CreateAutoCodeHistory(meta, structName, structCNName, autoCodePath string, injectionMeta string, tableName string, apiIds string) error {
	return global.GVA_DB.Create(&types.SysAutoCodeHistory{
		RequestMeta:   meta,
...
func (autoCodeHistoryService *AutoCodeHistoryService) GetMeta(id uint) (string, error) {
	var meta string
	return meta, global.GVA_DB.Model(types.SysAutoCodeHistory{}).Select("request_meta").First(&meta, id).Error
}

// GetSysHistoryPage  获取系统历史数据
func (autoCodeHistoryService *AutoCodeHistoryService) GetSysHistoryPage(info types.PageInfo) (err error, list interface{}, total int64) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	db := global.GVA_DB
	var fileLists []types.SysAutoCodeHistory
...
func (autoCodeHistoryService *AutoCodeHistoryService) DeletePage(id uint) error {
	return global.GVA_DB.Delete(types.SysAutoCodeHistory{}, id).Error
}
