...
var SimpleUploaderServiceApp = new(SimpleUploaderService)
...
func (exa *SimpleUploaderService) SaveChunk(uploader types.ExaSimpleUploader) (err error) {
...
func (exa *SimpleUploaderService) CheckFileMd5(md5 string) (err error, uploads []types.ExaSimpleUploader, isDone bool) {
	err = global.GVA_DB.Find(&uploads, "identifier = ? AND is_done = ?", md5, false).Error
	isDone = errors.Is(global.GVA_DB.First(&types.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, true).Error, gorm.ErrRecordNotFound)
...
func (exa *SimpleUploaderService) MergeFileMd5(md5 string, fileName string) (err error) {
	...
	if !errors.Is(global.GVA_DB.First(&types.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, true).Error, gorm.ErrRecordNotFound) {
		return nil
	}
	...
	err = global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		// 删除切片信息
		if err = tx.Delete(&types.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, false).Error; err != nil {
			fmt.Println(err)
			return err
		}
		data := types.ExaSimpleUploader{
			Identifier: md5,
			IsDone:     true,
